...
namespace Acczite20.Services.Reporting
{
    ...
    public class GstReportService
    {
        ...
        public async Task<List<GstRateWiseSummary>> GetRateWiseSummaryAsync(Guid orgId, DateTime fromDate, DateTime toDate)
        {
            ...
            return breakdowns.GroupBy(g => g.TaxRate)
                ...
                {
                    TaxRate = group.Key,
                    TaxableValue = group.Select(g => g.AssessableValue).Distinct().Sum(), // This is tricky as assessable value might be repeated per tax type
