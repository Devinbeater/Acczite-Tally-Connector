...
namespace Acczite20.Services.Sync
{
    ...
    public class TallyMasterSyncService
    {
        ...
        // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        // DIMENSION TABLE POPULATION (Fixed GroupId resolution)
        // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â        private async Task PopulateDimLedgersAsync(Guid orgId, AppDbContext dbContext)
        {
            var ledgers = await dbContext.Ledgers.Where(l => l.OrganizationId == orgId).ToListAsync();
            var dimGroups = await dbContext.DimGroups.Where(d => d.OrganizationId == orgId).ToDictionaryAsync(d => d.GroupName);
            var existingDimLedgers = await dbContext.DimLedgers.Where(d => d.OrganizationId == orgId).ToDictionaryAsync(d => d.Id);

            foreach (var l in ledgers)
            {
                // Resolve GroupId from DimGroups â€” THIS WAS THE CRITICAL BUG (was always Guid.Empty)
                Guid groupId = Guid.Empty;
                if (dimGroups.TryGetValue(l.ParentGroup, out var dimGroup))
                {
                    groupId = dimGroup.Id;
                }

                if (existingDimLedgers.TryGetValue(l.Id, out var existingDim))
                {
                    existingDim.LedgerName = l.Name;
                    existingDim.GroupId = groupId;
                    existingDim.ParentGroupName = l.ParentGroup;
                    existingDim.IsBillWise = l.IsBillWise;
                    existingDim.IsGstLedger = !string.IsNullOrEmpty(l.GSTApplicability)
                                             && !string.Equals(l.GSTApplicability, "Not Applicable", StringComparison.OrdinalIgnoreCase);
                    existingDim.TallyMasterId = l.TallyMasterId;
                    existingDim.UpdatedAt = DateTimeOffset.UtcNow;
                }
                else
                {
                    await dbContext.DimLedgers.AddAsync(new Acczite20.Models.Warehouse.DimLedger
                    {
                        ...
                        LedgerName = l.Name,
                        GroupId = groupId,
                        ParentGroupName = l.ParentGroup,
                        IsBillWise = l.IsBillWise,
                        IsGstLedger = !string.IsNullOrEmpty(l.GSTApplicability) 
                                    && !string.Equals(l.GSTApplicability, "Not Applicable", StringComparison.OrdinalIgnoreCase),
                        TallyMasterId = l.TallyMasterId,
                        CreatedAt = DateTimeOffset.UtcNow,
                        UpdatedAt = DateTimeOffset.UtcNow
                    });
                }
            ...
            await dbContext.SaveChangesAsync();
        }gDim.ParentGroupName = l.ParentGroup;
                    existingDim.IsBillWise = l.IsBillWise;
                    existingDim.IsGstLedger = !string.IsNullOrEmpty(l.GSTApplicability)
                                            && !string.Equals(l.GSTApplicability, "Not Applicable", StringComparison.OrdinalIgnoreCase);
                    existingDim.TallyMasterId = l.TallyMasterId;
                ...
                {
                    await _context.DimLedgers.AddAsync(new Acczite20.Models.Warehouse.DimLedger
                    {
                        Id = l.Id,
                        OrganizationId = orgId,
                        LedgerName = l.Name,
