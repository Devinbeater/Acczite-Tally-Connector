...
new_func = """        private async Task SyncLedgersFromXmlAsync(Guid orgId, AppDbContext dbContext)
        {
            try
            {
                ...
                foreach (var node in nodes)
                {
                    ...
                    if (existingLedgers.TryGetValue(tallyId, out var existing))
                    {
                        ...
                        {
                             _syncMonitor.AddLog($"❌ CRITICAL: Ledger '{ledger.Name}' points to missing Group '{ledger.ParentGroup}'.", "ERROR", "MASTERS");
                        ...
                        if (Math.Abs(existing.ClosingBalance - ledger.ClosingBalance) > 0.01m)
                        {
                            existing.ClosingBalance = ledger.ClosingBalance;
                        }
                    }
                    else
                    {
                    ...
                    count++;
                    if (count % 1000 == 0) await dbContext.SaveChangesAsync();
                }
                        ...
                        try
                        {
                            ...
                            foreach (var dNode in dNodes)
                            {
                                ...
                                if (existingLedgers.TryGetValue(masterId, out var existing))
                                {
                                    ...
                                    existing.PartyGstIn = detailLedger.PartyGstIn;
                                    existing.IncomeTaxNumber = detailLedger.IncomeTaxNumber;
                                    existing.LedgerStateName = detailLedger.LedgerStateName;
                                    existing.Email = detailLedger.Email;
                                    existing.IsBillWiseOn = detailLedger.IsBillWiseOn;
                                    existing.LedgerContact = detailLedger.LedgerContact;
            ...
            catch (Exception ex)
            {
                ...
                _syncMonitor.AddLog($"❌ XML Ledger sync failed: {ex.Message}", "ERROR", "MASTERS");
                throw;
            }
            finally
            {
